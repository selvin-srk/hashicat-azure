...
public class ApiHandler : IHttpHandler
{
    ...
    // ======================================================================
    //  HELPERS  — all written in C# 5.0 syntax (no expression bodies, no $"")
    // ======================================================================
    ...
    private object GetFilterOptions()
    {
        ...
        using (SqlConnection cn = Conn())
        {
            ...
            {
                "SELECT DISTINCT Event FROM [REMEDY_Info].[dbo].[GOALogins] WHERE Event IS NOT NULL ORDER BY Event",
                "SELECT DISTINCT AdsSiteName FROM [REMEDY_Info].[dbo].[GOALogins] WHERE AdsSiteName IS NOT NULL ORDER BY AdsSiteName",
                "SELECT DISTINCT MachineOS FROM [REMEDY_Info].[dbo].[GOALogins] WHERE MachineOS IS NOT NULL ORDER BY MachineOS"
            };
            List<string>[] lists = new List<string>[] { events, sites, osList };

            for (int i = 0; i < sqls.Length; i++)
            {
                using (SqlCommand cmd = new SqlCommand(sqls[i], cn))
                using (SqlDataReader r = cmd.ExecuteReader())
                    while (r.Read())
                        if (!r.IsDBNull(0)) lists[i].Add(r[0].ToString().Trim());
            }
        }
        return new { events = events, sites = sites, os = osList };
    }
...
GROUP BY LogonWeekday";
        ...
        List<string> labels = new List<string>(order);
        List<long>   data   = new List<long>();
...
GROUP BY YEAR(LogonDate) ORDER BY Yr";
    ...
    private object GetLiveFeed(HttpContext ctx)
    {
        ...
        using (SqlConnection cn = Conn())
        {
            string cntSql = "SELECT COUNT(*) FROM [REMEDY_Info].[dbo].[GOALogins] " + where.ToString();
            using (SqlCommand cmd = new SqlCommand(cntSql, cn))
            {
                AddParams(cmd, from, to);
...
GROUP BY YEAR(LogonDate), MONTH(LogonDate) ORDER BY Yr, Mo";
            using (SqlCommand cmd = new SqlCommand(monthlySql, cn))
            {
...
WHERE LastLogonID = @u AND LogonDate >= @from AND LogonDate < DATEADD(day,1,@to)
GROUP BY DATEPART(hour, TRY_CONVERT(time, LogonTime))";
            using (SqlCommand cmd = new SqlCommand(hourlySql, cn))
            {
                cmd.Parameters.AddWithValue("@u", userId);
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
...
GROUP BY MachineName ORDER BY Cnt DESC";
            using (SqlCommand cmd = new SqlCommand(machSql, cn))
            {
                cmd.Parameters.AddWithValue("@u", userId);
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
...
WHERE LastLogonID = @u AND LogonDate >= @from AND LogonDate < DATEADD(day,1,@to)
ORDER BY LogonDate DESC, LogonTime DESC";
            using (SqlCommand cmd = new SqlCommand(evSql, cn))
            {
                cmd.Parameters.AddWithValue("@u", userId);
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
...
ORDER BY LogonDate DESC, LogonTime DESC";
            using (SqlCommand cmd = new SqlCommand(ahSql, cn))
            {
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
                    while (r.Read())
...
ORDER BY LogDate DESC, SiteCount DESC";
            using (SqlCommand cmd = new SqlCommand(msSql, cn))
            {
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
                    while (r.Read())
...
ORDER BY LoginCount DESC";
            using (SqlCommand cmd = new SqlCommand(hfSql, cn))
            {
                AddParams(cmd, from, to);
                using (SqlDataReader r = cmd.ExecuteReader())
                    while (r.Read())
        ...
        // Anomaly trend (after-hours per day, last 30 days)
...
GROUP BY CAST(LogonDate AS DATE) ORDER BY Day";
        using (SqlConnection cn = Conn())
        using (SqlCommand cmd = new SqlCommand(trendSql, cn))
        using (SqlDataReader r = cmd.ExecuteReader())
        ...
        foreach (Dictionary<string, object> a in alerts)
        {
            ...
            string sv = a["severity"].ToString();
            if (sv == "critical") critCount++; else warnCount++;
    ...
    private object GetMachines(HttpContext ctx)
    {
        ...
        using (SqlConnection cn = Conn())
        {
            string cntSql = "SELECT COUNT(DISTINCT MachineName) FROM [REMEDY_Info].[dbo].[GOALogins] " + where.ToString();
            using (SqlCommand cmd = new SqlCommand(cntSql, cn))
            {
                AddParams(cmd, from, to);
                if (search != "") cmd.Parameters.AddWithValue("@search", "%" + search + "%");
    ...
    private object GetComparison(HttpContext ctx)
    {
        ...
        using (SqlConnection cn = Conn())
        {
            ...
            string mSql = "SELECT YEAR(LogonDate) AS Yr, MONTH(LogonDate) AS Mo, " +
                          ...
                          "GROUP BY YEAR(LogonDate),MONTH(LogonDate) ORDER BY Yr,Mo";
            using (SqlCommand cmd = new SqlCommand(mSql, cn))
            {
            ...
            string hSql = "SELECT DATEPART(hour,TRY_CONVERT(time,LogonTime)) AS Hr, " +
                          ...
                          "WHERE (" + col + "=@a OR " + col + "=@b) AND LogonDate>=@from AND LogonDate<DATEADD(day,1,@to) " +
                          "GROUP BY DATEPART(hour,TRY_CONVERT(time,LogonTime))";
            using (SqlCommand cmd = new SqlCommand(hSql, cn))
            {
                cmd.Parameters.AddWithValue("@a", entityA);
                cmd.Parameters.AddWithValue("@b", entityB);
                AddParams(cmd, from, to);
            ...
            string wSql = "SELECT LogonWeekday, " +
                          ...
                          "WHERE (" + col + "=@a OR " + col + "=@b) AND LogonDate>=@from AND LogonDate<DATEADD(day,1,@to) " +
                          "GROUP BY LogonWeekday";
            using (SqlCommand cmd = new SqlCommand(wSql, cn))
            {
                cmd.Parameters.AddWithValue("@a", entityA);
                cmd.Parameters.AddWithValue("@b", entityB);
                AddParams(cmd, from, to);
