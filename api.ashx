...
public class ApiHandler : IHttpHandler
{
    ...
    // How long to cache aggregation results (minutes). Increase if DB load is high.
    private const int CACHE_MINUTES = 5;

    private readonly JavaScriptSerializer _json = new JavaScriptSerializer();

    // ======================================================================
    //  CACHE HELPER — stores any action result keyed by action + parameters
    // ======================================================================
    private object GetCached(HttpContext ctx, string cacheKey)
    {
        return ctx.Cache[cacheKey];
    }

    private void SetCached(HttpContext ctx, string cacheKey, object value, int minutes)
    {
        ctx.Cache.Insert(cacheKey, value, null,
            DateTime.Now.AddMinutes(minutes), Cache.NoSlidingExpiration);
    }

    private string CacheKey(string action, DateTime from, DateTime to)
    {
        return "GOA_" + action + "_" + from.ToString("yyyyMMdd") + "_" + to.ToString("yyyyMMdd");
    }
    ...
    private object GetKPI(HttpContext ctx)
    {
        ...
        string ck = CacheKey("kpi", from, to);
        object cached = GetCached(ctx, ck);
        if (cached != null) return cached;
...
WHERE LogonDate >= @from AND LogonDate < DATEADD(day,1,@to)";
        ...
        SetCached(ctx, ck, kpiResult, CACHE_MINUTES);
        return kpiResult;
    }
    ...
    private object GetDailyVolume(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("daily", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
...
GROUP BY CAST(LogonDate AS DATE) ORDER BY Day";
        ...
        object result = new { labels = labels, data = data };
        SetCached(ctx, ck, result, CACHE_MINUTES);
        return result;
    }
    ...
    private object GetEventTypes(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("events", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
...
GROUP BY Event ORDER BY Cnt DESC";
    ...
    private object GetWeekdayDist(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("weekday", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
...
GROUP BY LogonWeekday";
        ...
        object wdResult = new { labels = new List<string>(order), data = data };
        SetCached(ctx, ck, wdResult, CACHE_MINUTES);
        return wdResult;
    }
    ...
    private object GetHourlyDist(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("hourly", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
...
GROUP BY DATEPART(hour, TRY_CONVERT(time, LogonTime))";
        ...
        object hrResult = new { labels = labels, data = data };
        SetCached(ctx, ck, hrResult, CACHE_MINUTES);
        return hrResult;
    }
...
WHERE YEAR(LogonDate) = @year AND LogonWeekday IS NOT NULL");
    ...
    private object GetSites(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("sites", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
...
GROUP BY AdsSiteName ORDER BY Cnt DESC";
    ...
    private object GetServers(HttpContext ctx)
    {
        DateTime from = DateP(ctx, "from", DateTime.Now.AddDays(-30));
        DateTime to   = DateP(ctx, "to",   DateTime.Now);
        string ck = CacheKey("servers", from, to);
        object cached = GetCached(ctx, ck); if (cached != null) return cached;
